...
namespace MOBILEAPI2024.API.Controllers
{
    ...
    public class ExitController : ControllerBase
    {
                ...
                if (AuthenticationHeaderValue.TryParse(authorization, out var headerValue))
                {
                    ...
                    if (jsonToken != null)
                    {
                        ...
                        if (!string.IsNullOrEmpty(empId) && !string.IsNullOrEmpty(cmpId))
                        {
                            ...
                            var compOffResponse = _exitService.ExitAppInsert(exitAppInsertRequest);
                            if (compOffResponse != null)
                            {
                                response.code = StatusCodes.Status200OK;
                                response.status = true;
        ...
        public IActionResult ExitApplicationNoticePeriod(ExitApplicationDeleteRequest exitApplicationDeleteRequest)
        {
            Response response = new Response();
            try
            {
